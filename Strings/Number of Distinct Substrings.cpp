 ...
 * Algorithm: Number of distinct substrings in a string (O(nlog^2n)).
 */
...
int main()
{
	...
	while(t--)
	{
		...
		suffix_array.init(word);
		ll ans = word.length() - suffix_array.sorted_sufs[0].f;
		for(ll i=1; i<word.length(); i++)
		{
			ans += word.length() - suffix_array.sorted_sufs[i].f - suffix_array.lcp(suffix_array.sorted_sufs[i].f, suffix_array.sorted_sufs[i-1].f);
		}

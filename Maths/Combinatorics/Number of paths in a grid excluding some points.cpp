 ...
 * 			The number of ways to reach a point in a grid from the starting
 *			point moving only right and down excluding some points.
 *
 * Solution: Let the points to be excluded be A1, A2, ... A(n-1). Let the 
  			 point to be reached be An. Now for each Ai we compute (Di) the
  			 number of ways to reach that point without going through any 
  			 of the previous points. Clearly Dn is the answer then.

  			 Now, we know that the number of ways to reach a point in a grid
  			 if we have to go x steps down and y steps right is (x+y)Cx. Now,
  			 from this we substract the number of ways to reach from any
  			 previous point, i.e. D(j)*(Number of ways) for each j<i.
 */
...
int main(){
	 ...
	 * w = width of the grid.
	 * n = number of cells to be excluded.
	 */
	ll h, w, n;
	cin>>h>>w>>n;

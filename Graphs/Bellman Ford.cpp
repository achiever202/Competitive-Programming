...
class BellmanFord
{
	...
	public:
		void init(ll source)
		{
			...
			for(ll i=0; i<graph.node_size-1; i++)
				for(ll j=1; j<=graph.node_size; j++)
					for(ll k=0; k<graph.nodes[j].edges.size(); k++)
						if(cost[graph.nodes[j].edges[k].node]>cost[j]+graph.nodes[j].edges[k].cost)
							cost[graph.nodes[j].edges[k].node] = cost[j]+graph.nodes[j].edges[k].cost;
		}
